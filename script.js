...
(() => {
  ...
  const RANGES = {
    "easy-1oct": { startOctave: 4, octaves: 1 },
    "med-2oct":  { startOctave: 3, octaves: 2 },
  ...
  const CHORD_TYPES = {
    ...
    min7:  { label: "Minor 7",    intervals: [0, 3, 7, 10], color: "#4da3ff" },
    dim7:  { label: "Diminished 7",intervals:[0, 3, 6, 9],  color: "#8a2be2" },
    m7b5:  { label: "Half-Dim 7", intervals: [0, 3, 6, 10], color: "#d2691e" },
    mmaj7: { label: "Min-Maj 7",  intervals: [0, 3, 7, 11], color: "#e83e8c" }
  };

  let activeChordKeys = ["maj7", "dom7"]; // Default selected
  ...
  const chordToggles = document.querySelectorAll("#chordToggles input[type='checkbox']");
  ...
  [infoModal, introModal].forEach((m) => {
    m?.addEventListener("click", (e) => {
      // Prevent closing intro modal by clicking outside since settings are there now
  ...
  function buildDynamicButtons() {
    ...
    activeChordKeys.forEach((key, idx) => {
      ...
      ansBtn.innerHTML = `${typeData.label} <span style="opacity:0.5; font-size:11px;">(${shortcutNum})</span>`;
  ...
  function updateActiveChordsFromToggles() {
    ...
    // Prevent unchecking if it brings us below 2 choices
    chordToggles.forEach(cb => {
      if (cb.checked && activeChordKeys.length <= 2) {
        cb.disabled = true;
      } else {
        cb.disabled = false;
      }
    ...
    if (activeChordKeys.length < 2) {
      activeChordKeys = ["maj7", "dom7"]; 
    }
  ...
  chordToggles.forEach(cb => cb.addEventListener("change", updateActiveChordsFromToggles));
  ...
  function computeRootBounds() {
    ...
    // Set max root to C#5 (pitch class 1, octave 5)
    // C#5 + a Major 7th interval (11 semitones) perfectly hits C6.
    const maxRoot = pitchFromPcOct(11, 4); 
  ...
  function updateScoreMetadata() {
    const metaStr = `Included: ${activeModeString()}`;
    if ($("scoreMeta")) $("scoreMeta").textContent = metaStr;
    if ($("modalScoreMeta")) $("modalScoreMeta").textContent = metaStr;
  }
  ...
  function computeTwoOctaveWindowForSet(pitches) {
    ...
    // Clamp the visual keyboard directly to C6, instead of C6 + 12
  ...
  function bind() {
    introBeginBtn?.addEventListener("click", () => {
      playUiSound(UI_SND_SELECT);
      updateActiveChordsFromToggles(); // ensure choices carry over
    ...
    document.addEventListener("keydown", async (e) => {
      ...
      // Dynamic keyboard shortcuts (1-6) mapping to active chords
      const numMatch = e.key.match(/^[1-6]$/);
      if (numMatch && canAnswer) {
        const index = parseInt(numMatch[0], 10) - 1;
        if (index < activeChordKeys.length) {
          answer(activeChordKeys[index]);
          return;
        }
      }
